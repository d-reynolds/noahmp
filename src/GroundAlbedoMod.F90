...
! local variable
   ...
   !$acc parallel loop collapse(2) gang vector default(present) private(IndSwBnd,AlbedoSoilAdjWet)
    do J = noahmp%config%domain%JTS, noahmp%config%domain%JTE
      do I = noahmp%config%domain%ITS, noahmp%config%domain%ITE


    ! solar radiation process is only done if there is light
    if ( CosSolarZenithAngle(I,J) <= 0 ) cycle
    ...
    do IndSwBnd = 1, NumSwRadBand
       ...
       if ( SurfaceType(I,J) == 1 )  then                      ! soil
       ...
       elseif ( TemperatureGrd(I,J) > ConstFreezePoint ) then  ! unfrozen lake, wetland
          AlbedoSoilDir(I,IndSwBnd,J) = 0.06 / (max(0.01, CosSolarZenithAngle(I,J))**1.7+0.15)
       ...
       AlbedoGrdDir(I,IndSwBnd,J) = AlbedoSoilDir(I,IndSwBnd,J)*(1.0-SnowCoverFrac(I,J)) + AlbedoSnowDir(I,IndSwBnd,J)*SnowCoverFrac(I,J)
       AlbedoGrdDif(I,IndSwBnd,J) = AlbedoSoilDif(I,IndSwBnd,J)*(1.0-SnowCoverFrac(I,J)) + AlbedoSnowDif(I,IndSwBnd,J)*SnowCoverFrac(I,J)
