...
! local variable
    ...
    !$acc parallel loop collapse(2) gang vector default(present) private(I,LoopInd,J)
   ...
   enddo
    ...
    !$acc parallel loop collapse(2) gang vector default(present) private(I,LoopInd,J) private(SnowDensBulk) &
    !$acc firstprivate(GlacierExcessRemainFrac)
    do J = noahmp%config%domain%JTS, noahmp%config%domain%JTE
      do I = noahmp%config%domain%ITS, noahmp%config%domain%ITE
   ...
   enddo
    ...
    !$acc parallel loop gang vector collapse(2) default(present) private(I,LoopInd,J)
    do J = noahmp%config%domain%JTS, noahmp%config%domain%JTE
      do I = noahmp%config%domain%ITS, noahmp%config%domain%ITE
