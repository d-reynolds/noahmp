...
! --------------------------------------------------------------------
   ...
   !$acc private(LoopInd,SnowBurden,SnowCompactAgeExpFac,TempDiff,SnowVoid,SnowWatTotTmp,SnowIceDens,SnowCompactBurdenFac)
    ...
    SnowCompactBurdenFac = SnowCompactm(I,J)  * TemperatureAirRefHeight(I,J) + SnowCompactb(I,J)
    ! pressure-based lower constraints:
    if ( PressureAirRefHeight(I,J) >= 85000.0 ) then ! high pressure bin
        SnowCompactBurdenFac = max(SnowCompactBurdenFac, SnowCompactPSFC1(I,J))
    endif
    if ( (PressureAirRefHeight(I,J) >= 80000.0) .and. (PressureAirRefHeight(I,J) < 85000.0) ) then ! mid-pressure bin
        SnowCompactBurdenFac = max(SnowCompactBurdenFac, SnowCompactPSFC2(I,J))
    endif
    if ( PressureAirRefHeight(I,J) < 80000.0 ) then ! low pressure bin
        SnowCompactBurdenFac = max(SnowCompactBurdenFac, SnowCompactPSFC3(I,J))
    endif
    ! upper constraint on SnowCompactBurdenFac
    SnowCompactBurdenFac = min(SnowCompactBurdenFac, BurdenFacUpper(I,J))
    ...
    do LoopInd = NumSnowLayerNeg(I,J)+1, 0
       ...
       if ( (SnowVoid > 0.001) .and. (SnowIce(I,LoopInd,J) > 0.1) ) then
          ...
          CompactionSnowBurden(I,LoopInd,J) = -(SnowBurden + 0.5*SnowWatTotTmp) * &
                                        exp(-0.08*TempDiff-SnowCompactBurdenFac*SnowIceDens) / SnowViscosityCoeff(I,J)  ! 0.5*SnowWatTotTmp -> self-burden
