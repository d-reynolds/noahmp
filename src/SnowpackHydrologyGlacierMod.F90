...
! local variables
    ...
    enddo
    SnowBotOutflow(I,J)     = 0.0
    InflowSnowLayer    = 0.0

    ! for the case when SnowWaterEquiv becomes '0' after 'COMBINE'
    if ( SnowWaterEquiv(I,J) == 0.0 ) then
       if ( OptGlacierTreatment == 1 ) then
          SoilIce(I,1,J) = SoilIce(I,1,J) + (FrostSnowSfcIce(I,J)-SublimSnowSfcIce(I,J)) * MainTimeStep / &
                           (ThicknessSnowSoilLayer(I,1,J)*1000.0)
       elseif ( OptGlacierTreatment == 2 ) then
          HeatSensibleSfc(I,J)  = HeatSensibleSfc(I,J) - (FrostSnowSfcIce(I,J) - SublimSnowSfcIce(I,J)) * ConstLatHeatSublim
    ...
    if ( (NumSnowLayerNeg(I,J) == 0) .and. (SnowWaterEquiv(I,J) > 0.0) ) then
       if ( OptGlacierTreatment == 1 ) then
          SnowWaterTmp   = SnowWaterEquiv(I,J)
          SnowWaterEquiv(I,J) = SnowWaterEquiv(I,J) - SublimSnowSfcIce(I,J)*MainTimeStep + FrostSnowSfcIce(I,J)*MainTimeStep
          SnowWaterRatio = SnowWaterEquiv(I,J) / SnowWaterTmp
       ...
       if ( SnowWaterEquiv(I,J) < 0.0 ) then
          SoilIce(I,1,J)      = SoilIce(I,1,J) + SnowWaterEquiv(I,J) / (ThicknessSnowSoilLayer(I,1,J)*1000.0)
    ...
    if ( (SnowDepth(I,J) <= 1.0e-8) .or. (SnowWaterEquiv(I,J) <= 1.0e-6) ) then
       SnowDepth(I,J)      = 0.0
       SnowWaterEquiv(I,J) = 0.0
    endif
    ...
    if ( NumSnowLayerNeg(I,J) < 0 ) then
       SnowWaterTmp = SnowIce(I,NumSnowLayerNeg(I,J)+1,J) + SnowLiqWater(I,NumSnowLayerNeg(I,J)+1,J) ! top layer total snow water before sublimation
       SnowIceTmp   = SnowIce(I,NumSnowLayerNeg(I,J)+1,J) - SublimSnowSfcIce(I,J)*MainTimeStep + FrostSnowSfcIce(I,J)*MainTimeStep
       SnowIce(I,NumSnowLayerNeg(I,J)+1,J) = SnowIceTmp
       if ( (SnowIceTmp < 1.0e-6) .and. (NumSnowLayerNeg(I,J) < 0) ) call SnowLayerCombine(noahmp, I, J)
       if ( (SnowIceTmp >= 1.0e-6) .and. (NumSnowLayerNeg(I,J) < 0) ) then ! re-adjust snow layer thickness
          ThicknessSnowSoilLayer(I,NumSnowLayerNeg(I,J)+1,J) = ThicknessSnowSoilLayer(I,NumSnowLayerNeg(I,J)+1,J) * &
                        (SnowIce(I,NumSnowLayerNeg(I,J)+1,J) + SnowLiqWater(I,NumSnowLayerNeg(I,J)+1,J)) / SnowWaterTmp ! assuming same snow density
       endif
       if ( NumSnowLayerNeg(I,J) < 0 ) then
          SnowLiqWater(I,NumSnowLayerNeg(I,J)+1,J) = SnowLiqWater(I,NumSnowLayerNeg(I,J)+1,J) + RainfallGround(I,J) * MainTimeStep
          SnowLiqWater(I,NumSnowLayerNeg(I,J)+1,J) = max(0.0, SnowLiqWater(I,NumSnowLayerNeg(I,J)+1,J))
       endif
    endif
    ...
    do LoopInd = NumSnowLayerNeg(I,J)+1, 0
       ...
       OutflowSnowLayer(I,LoopInd,J)= max(0.0, (SnowLiqWaterVol(I,LoopInd,J) - SnowLiqHoldCap(I,J)*SnowEffPorosity(I,LoopInd,J)) * &
       ...
       if ( LoopInd == 0 ) then
          OutflowSnowLayer(I,LoopInd,J) = max((SnowLiqWaterVol(I,LoopInd,J)-SnowEffPorosity(I,LoopInd,J)) * ThicknessSnowSoilLayer(I,LoopInd,J), &
                                          SnowLiqReleaseFac(I,J) * MainTimeStep * OutflowSnowLayer(I,LoopInd,J))
       endif
       ...
       if ( ( SnowLiqWater(I,LoopInd,J) / (SnowIce(I,LoopInd,J)+SnowLiqWater(I,LoopInd,J)) ) > SnowLiqFracMax(I,J) ) then
          OutflowSnowLayer(I,LoopInd,J) = OutflowSnowLayer(I,LoopInd,J) + &
                                          (SnowLiqWater(I,LoopInd,J) - SnowLiqFracMax(I,J)/(1.0-SnowLiqFracMax(I,J)) * SnowIce(I,LoopInd,J))
          SnowLiqWater(I,LoopInd,J)     = SnowLiqFracMax(I,J) / (1.0 - SnowLiqFracMax(I,J)) * SnowIce(I,LoopInd,J)
       endif
    ...
    SnowBotOutflow(I,J) = OutflowSnowLayer(I,0,J) / MainTimeStep
    !$acc loop seq
    do LoopInd = NumSnowLayerNeg(I,J)+1, 0
