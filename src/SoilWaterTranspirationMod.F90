...
! --------------------------------------------------------------------
    associate(                                                                             &
              ...
              SoilMatPotentialWilt      => noahmp%water%param%SoilMatPotentialWilt(I,J)        ,& ! in,  soil metric potential for wilting point [m]
              ...
              SoilTranspFacAcc          => noahmp%water%state%SoilTranspFacAcc(I,J)            ,& ! out, accumulated soil water transpiration factor (0 to 1)
    ...
    if ( SurfaceType ==1 ) then
       ...
       do IndSoil = 1, NumSoilLayerRoot
          ...
          if ( OptSoilWaterTranspiration == 2 ) then  ! CLM
             SoilMatPotential(I,IndSoil,J) = max(SoilMatPotentialWilt, -SoilMatPotentialSat(I,IndSoil,J) * &
                                                (max(0.01,SoilLiqWater(I,IndSoil,J))/SoilMoistureSat(I,IndSoil,J)) ** &
                                                (-SoilExpCoeffB(I,IndSoil,J)))
             SoilWetFac                = (1.0 - SoilMatPotential(I,IndSoil,J)/SoilMatPotentialWilt) / &
                                         (1.0 + SoilMatPotentialSat(I,IndSoil,J)/SoilMatPotentialWilt)
          endif
          if ( OptSoilWaterTranspiration == 3 ) then  ! SSiB
             SoilMatPotential(I,IndSoil,J) = max(SoilMatPotentialWilt, -SoilMatPotentialSat(I,IndSoil,J) * &
                                                (max(0.01,SoilLiqWater(I,IndSoil,J))/SoilMoistureSat(I,IndSoil,J)) ** &
                                                (-SoilExpCoeffB(I,IndSoil,J)))
             SoilWetFac = 1.0 - exp(-5.8*(log(SoilMatPotentialWilt/SoilMatPotential(I,IndSoil,J))))
          endif
          SoilWetFac    = min(1.0, max(0.0,SoilWetFac))

          SoilTranspFac(I,IndSoil,J) = max(MinThr, ThicknessSnowSoilLayer(I,IndSoil,J) / &
                                          (-DepthSoilLayer(I,NumSoilLayerRoot,J)) * SoilWetFac)
          SoilTranspFacAcc           = SoilTranspFacAcc + SoilTranspFac(I,IndSoil,J)
       enddo
