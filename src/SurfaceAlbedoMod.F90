...
! --------------------------------------------------------------------
   ...
   !$acc LightExtDir) firstprivate(MinThr)
    ...
    if ( CosSolarZenithAngle(I,J) > 0.0 ) then

       ! weight reflectance/transmittance by LeafAreaIndex and StemAreaIndex
       LeafWgt = LeafAreaIndEff(I,J) / max(VegAreaIndEff(I,J), MinThr)
       StemWgt = StemAreaIndEff(I,J) / max(VegAreaIndEff(I,J), MinThr)
       !$acc loop seq
       do IndBand = 1, NumSwRadBand
   ...
   !$acc private(IndBand, IndDif, LeafWgt, StemWgt, LightExtDir) firstprivate(MinThr)
    ...
    if ( CosSolarZenithAngle(I,J) > 0.0 ) then
       ...
       LightExtDir      = VegAreaProjDir(I,J) / CosSolarZenithAngle(I,J) * sqrt(1.0-ReflectanceVeg(I,1,J)-TransmittanceVeg(I,1,J))
       CanopySunlitFrac(I,J) = (1.0 - exp(-LightExtDir*VegAreaIndEff(I,J))) / max(LightExtDir*VegAreaIndEff(I,J), MinThr)
       LightExtDir      = CanopySunlitFrac(I,J)
       if ( LightExtDir < 0.01 ) then
          LeafWgt = 0.0
       else
          LeafWgt = LightExtDir
       endif
       CanopySunlitFrac(I,J) = LeafWgt

    endif  ! CosSolarZenithAngle(I,J) > 0

    ! shaded canopy fraction
    CanopyShadeFrac(I,J)   = 1.0 - CanopySunlitFrac(I,J)
    LeafAreaIndSunlit(I,J) = LeafAreaIndEff(I,J) * CanopySunlitFrac(I,J)
    LeafAreaIndShade(I,J)  = LeafAreaIndEff(I,J) * CanopyShadeFrac(I,J)
