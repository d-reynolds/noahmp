...
! -----------------------------------------------------------------
   !$acc parallel loop collapse(2) gang vector present(noahmp) private(IndBand,IndLoop,RadSwAbsGrdTmp,RadSwReflGrdTmp,FracRadSwAbsSnowDirMean,FracRadSwAbsSnowDifMean)
...
! ----------------------------------------------------------------------
    ...
    do IndBand = 1, NumSwRadBand
       ...
       if ( OptSnowAlbedo == 3 ) then
          !$acc loop seq
          do IndLoop = -NumSnowLayerMax+1, 1
             FracRadSwAbsSnowDirMean = FracRadSwAbsSnowDir(I,IndLoop,IndBand,J) * SnowCoverFrac +    &
                                       ((1.0 - SnowCoverFrac) * (1.0 - AlbedoLandIce(I,IndBand,J)) * &
                                       (FracRadSwAbsSnowDir(I,IndLoop,IndBand,J)/(1.0-AlbedoSnowDir(I,IndBand,J))))
             FracRadSwAbsSnowDifMean = FracRadSwAbsSnowDif(I,IndLoop,IndBand,J) * SnowCoverFrac +    &
                                       ((1.0 - SnowCoverFrac) * (1.0 - AlbedoLandIce(I,IndBand,J)) * &
